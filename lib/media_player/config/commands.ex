defmodule MediaPlayer.Commands do
  ...
  def add() do
    title = IO.gets("Enter the title of the song: ") |> trim()
    album = IO.gets("Enter the album of the song: ") |> trim()
    artist = IO.gets("Enter the artist of the song: ") |> trim()
    created = IO.gets("Enter the date the song was created (YYYY-MM-DD): ") |> trim()
  ...
  def list() do
    ...
    |> Enum.each(fn %{
      ...
      IO.puts("ID: #{id} | Title: #{title} | Album: #{album} | Artist: #{artist} | Created At: #{created_at}")
    end)
  end
